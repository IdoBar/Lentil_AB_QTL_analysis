...
recent_vcf <- recent_file("../GBS_analysis/data/intermediate_files/", glue::glue(".*{stacks_name}.+\\.clean.vcf"))
...
vars_by_sample <- DataFrameList(lapply(parents,
                     function(s) getAllVariantsDF(snp_vcf[,s], txdb, var_types))) %>%
...
gmap <- readr::read_csv(glue::glue("./qtl2_files/Lentil_GBS_{stacks_name}_gmap.csv"))
