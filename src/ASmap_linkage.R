...
vcf_file <- "./data/hari_filtered_V1_min_head.recode.vcf"  # ref_stacks2_populations.snps.vcf
group_markers <- "LG"
vcf_basename <- tools::file_path_sans_ext(basename(vcf_file))
  
source("./src/vcf_filtration.R")
vcf_filtration(vcf_file, miss_rates = seq(0.2, 0.1, -0.05), geno_rate = 0.85, remove_hetero = FALSE, bi_allele_only = TRUE)
...
mapDHs <- pullCross(mapDHs, type = "seg.distortion", pars =
                      list(seg.ratio = "30:0:70"))
mapDHs <- pullCross(mapDHs, type = "missing", pars = list(miss.thresh = 0.1))
names(mapDHs)
